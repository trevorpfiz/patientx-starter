...
Medical history intake forms are time-consuming and a significant barrier to efficient digital patient care. A seamless third-party integration that can automatically pull this data based on a patient's personal information would be a game-changer. We don't have this capability in the starter currently, but medical history intake forms need to go by the wayside, stat! [Zus](https://zushealth.com/) integration with Canvas Medical may provide this functionality with their Zus Aggregated Profile, a shareable view of a patient that makes it quick and easy for care teams to understand all of a patient's recent and ongoing healthcare interactions. Another third-party integration that may simplify this process or provide broader access to patient data would be [Particle Health](https://www.particlehealth.com/), which can pipe in patient medical records from the largest healthcare networks. Additionally, with FHIR support, Canvas Medical can potentially interact directly with EHRs or QHINs that also use FHIR standards, facilitating access and importing patient health data directly into the Canvas platform. These systems often use OAuth2 or similar secure authorization frameworks that require explicit permission from the patient or the healthcare provider to access health records. Developers usually need credentials that are granted either by the healthcare organization or the EHR/QHIN provider. Canvas Medical needs to provide a secure and robust API infrastructure that can handle connections with EHRs and QHINs to make this possible.
