...
const SearchPractitioner = ({ setRecipient }: {
  setRecipient: Dispatch<SetStateAction<string>>
}) => {
  const [open, setOpen] = useState(false)
  const [value, setValue] = useState("")

  const { data } = api.practitioner.searchPractitioner.useQuery({
    query: {
    }
  })
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {value
            ? practitioners?.find((practitioner) => practitioner.name[0]!.text.toLowerCase() === value)?.name[0]!.text
            : "Select practitioner..."}
            ...
            {practitioners?.map((practitioner) => (
              ...
              >
                <Check
                  className={cn(
                    "mr-2 h-4 w-4",
                    value === practitioner.name[0]!.text ? "opacity-100" : "opacity-0"
                  )}
                />
